...
export default function SymbolControls({ settings, onSettingChange, onExport }) {
    ...
    <div className="border-t border-zinc-800/80 bg-zinc-900/60">
      ...
      <div className="px-5 py-4 flex flex-col gap-4">
        ...
        <SliderInput
          ...
          min={2} max={20}
          value={regionCount}
          onChange={handle('regionCount')}
        />
        <SliderInput
          label="Cell Size"
          min={4} max={64}
          value={cellSize}
          onChange={handle('cellSize')}
        />
        ...
        <div className="flex flex-col gap-3">
          <ToggleInput
            ...
            onChange={handleBool('showBoundaries')}
          />
          <ToggleInput
            label="Color Patterns with Region Average"
            checked={colorRegions}
            onChange={handleBool('colorRegions')}
          />
        </div>
        ...
        <button
          onClick={onExport}
          className="w-full mt-1 py-2 text-[10px] font-semibold uppercase tracking-[0.14em]
            border border-zinc-700 text-zinc-400
            hover:border-zinc-500 hover:text-zinc-200 hover:bg-zinc-800/50
